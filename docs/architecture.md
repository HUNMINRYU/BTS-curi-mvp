...
flowchart LR
  ...
  subgraph EC2["Amazon EC2 · ap-northeast-2 단일 인스턴스"]
    ...
    SIDECAR --> INDEX[("EC2-local active bundle<br/>index.faiss · chunks.jsonl")]
  end

  APP -->|"추천 이유 · 근거 기반 답변"| SONNET["Amazon Bedrock<br/>Claude Sonnet"]
  SIDECAR -->|"질의 임베딩"| TITAN["Amazon Bedrock<br/>Titan Text Embeddings v2"]
  S3[("Amazon S3<br/>private · versioned<br/>AES256 · public access blocked<br/>77 sanitized PDFs + 77 metadata")] -->|"build-time source/provenance"| INDEXER["RAG indexer<br/>rag_indexer.py"]
  INDEXER -->|"build-time PDF 청크 임베딩"| TITAN
  INDEXER -->|"build-time local bundle"| INDEX
```

학생 동선은 회원가입·로그인 → 온보딩 → 추천 → 시간표 → 과목 상세 → 질문·팁이고, 교수 동선은 읽기 전용 리포트 하나다. 정제 JSON은 앱 번들에 포함된 로컬 파일을 읽으며, 배포 런타임에 S3 JSON loader는 없다. 문서 원본과 metadata는 비공개(private)·versioned S3 bucket에서 build-time bundle의 source/provenance로 관리한다. S3 Public Access Block의 `BlockPublicAcls`, `IgnorePublicAcls`, `BlockPublicPolicy`, `RestrictPublicBuckets`는 모두 `true`이고, 기본 서버 측 암호화는 `AES256`, versioning 상태는 `Enabled`다.

현재 production Q&A는 EC2 로컬 FAISS bundle과 Bedrock을 사용한다. S3는 build-time 입력과 provenance 경계에 있고 요청마다 읽지 않는다. 앱 자체 회원가입은 제공하며, 관리형 인증·DB, 알림, 외부 강의평, 자유서술 커뮤니티, 별도 관리형 벡터 DB는 경계 밖이다.
...
5. 교수 가입은 server-only `CURI_PROFESSOR_SIGNUP_CODE`와 요청 코드를 비교한다. 환경변수가 없거나 비어 있으면 deny-closed로 HTTP 403을 반환하며, 코드를 교체한 뒤에는 앱 프로세스를 재시작해야 한다.
...
2. build-time `scripts/rag_indexer.py`는 비공개(private)·versioned·AES256 암호화·public access blocked bucket `hackathon-e2-t01-curi-docs`의 `documents/` 아래 sanitized PDF 77개와 `{pdf}.metadata.json` sidecar 77개를 읽어 Titan Text Embeddings v2 512-dimensional vectors와 로컬 `index.faiss`·`chunks.jsonl`을 만든다. 현재 bundle은 458 vectors와 77 courses다.
3. production의 `curi-rag.service`와 `rag_sidecar.py`는 로컬 `index.faiss`와 `chunks.jsonl`만 읽고, 질의 임베딩을 위해 Bedrock Runtime만 호출한다. sidecar에는 S3 client가 없으며 `CURI_DOCUMENT_BUCKET`도 읽지 않는다. 따라서 production query runtime은 S3에 요청하지 않는다.
4. Q&A 런타임은 `CURI_RAG_RETRIEVER_URL`(기본 `http://127.0.0.1:8788/retrieve`)에 `{ courseId, question, numberOfResults: 5 }`를 POST한다.
5. retriever는 `metadata.courseId`가 요청 `courseId`와 정확히 같은 결과만 `{ results: [...] }`로 반환한다. 각 결과는 청크 텍스트, source/provenance를 나타내는 `s3Uri` 문자열, `courseId`, `courseName`, `department`, 선택적 숫자 `week`를 포함한다. 이 문자열을 반환하는 것만으로 S3 요청이 발생하지는 않는다.
6. 검색 결과가 없으면 Bedrock Runtime을 호출하지 않고 `not_found`를 반환하며 질문을 `qa_logs`에 저장한다.
7. 웹 앱은 검색 청크의 인용문과 `s3Uri`에서 파싱한 문서 파일명을 근거로 전달한다. 런타임 답변에는 검색된 근거만 인용하며, 이 과정에서 S3 원본을 다시 읽지 않는다.
8. retriever 또는 모델 오류는 `model_error`와 이미 확보된 근거를 유지한다.
...
- 현재 production은 458 vectors·77 courses·512 dimensions의 사전 생성 로컬 FAISS bundle을 읽으며, `CURI_DOCUMENT_BUCKET`이나 S3 문서 원본을 런타임에서 사용하지 않는다.
- S3 `hackathon-e2-t01-curi-docs`는 비공개(private)·versioned·AES256 암호화·public access blocked build-time source/provenance다. `documents/` 아래 sanitized PDF 77개와 metadata sidecar 77개를 `scripts/rag_indexer.py`가 읽어 로컬 FAISS bundle을 만든다.
- `scripts/prepare-rag-documents.mjs`는 로컬 ZIP 경로에서 Git 밖 staging의 PDF와 `{filename}.metadata.json`을 생성한다. 이 명령은 production query runtime에 참여하지 않는다.
- 강의정보에 개인정보가 있으면 앱 시작 전 데이터 검증에서 실패한다.

## 7. 검증 경계
- 단위: 카탈로그 검증, 추천 비선호 제외·점수, 팁 임계치, RAG 과목 필터.
- API: 세션 역할, 프로필, 추천 폴백, 시간표 지속성, Q&A 세 상태·로그, 팁 중복.
- S3 source/provenance: `BlockPublicAcls=true`, `IgnorePublicAcls=true`, `BlockPublicPolicy=true`, `RestrictPublicBuckets=true`, 기본 서버 측 암호화 `AES256`, versioning `Enabled`, `documents/`의 77 PDF·77 metadata sidecar와 source URI prefix를 확인한다. 로컬 bundle은 458 records, 77 courses, 512 dimensions와 contact-pattern 부재를 확인한다.
...
- build-time source를 갱신하거나 bundle을 배포할 때 원본 zip·PDF·PEM·.env가 Git·EC2 배포 산출물에 없는지 검사한다.
